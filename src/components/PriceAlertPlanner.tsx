...
export default function PriceAlertPlanner({ stocks, alerts, onAddAlert, onDeleteAlert }: Props) {
  ...
  return (
    <div className="bg-white rounded-none border border-[#141414] p-4 mt-5">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <form onSubmit={handleCreateAlert} className="flex flex-wrap items-end gap-2.5 bg-[#E4E3E0]/40 p-2 border border-[#141414] w-full md:w-auto">
          <div className="flex-1 min-w-[100px]">
            <label className="block text-[9px] text-gray-600 font-mono uppercase mb-0.5">ТИКЕР</label>
            <input
              type="text"
              placeholder="AAPL, TSLA..."
              value={newTicker}
              onChange={e => setNewTicker(e.target.value)}
              className="w-full bg-white border border-[#141414] rounded-none px-2 py-1 text-xs text-[#141414] uppercase font-bold focus:outline-none"
            />
          </div>

          <div>
            <label className="block text-[9px] text-gray-600 font-mono uppercase mb-0.5">СИГНАЛ ПРИ</label>
            <select
              value={criteria}
              onChange={e => setCriteria(e.target.value as any)}
              className="bg-white border border-[#141414] rounded-none px-2 py-1 text-xs text-[#141414] font-extrabold focus:outline-none"
            >
          ...
          <div className="flex-1 min-w-[90px]">
            <label className="block text-[9px] text-gray-600 font-mono uppercase mb-0.5">ТАРГЕТ ЦЕНА ($)</label>
            <input
              type="number"
              step="0.01"
              placeholder="400.00"
              ...
              className="w-full bg-white border border-[#141414] rounded-none px-2 py-1 text-xs text-[#141414] font-mono focus:outline-none"
            />
          </div>

          <button
            type="submit"
            className="bg-black hover:bg-[#252525] text-white font-extrabold text-[10px] px-4 h-[26px] border border-[#141414] rounded-none flex items-center gap-1 transition-all uppercase"
          >
      ...
      {alerts.length > 0 && (
        <div className="mt-3.5 border-t border-[#141414]/20 pt-3">
          <h4 className="text-[10px] font-extrabold text-[#141414] mb-2 font-mono flex items-center gap-1.5 uppercase">
          ...
          <div className="flex flex-wrap gap-1.5 max-h-24 overflow-y-auto">
            {alerts.map(alert => (
              <div
                key={alert.id}
                className="bg-white border border-[#141414] rounded-none px-2 py-0.5 text-xs flex items-center gap-2 font-mono text-[#141414]"
              >
                ...
                <button
                  ...
                  className="text-gray-400 hover:text-red-700 ml-1.5 transition-colors cursor-pointer"
