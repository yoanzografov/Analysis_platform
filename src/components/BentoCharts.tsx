 ...
 Съотношение Купувай / Продавай / Изчакай
  ...
  <div className="group/info relative flex items-center -mt-2">
    ...
    <div className="absolute top-full mt-2 left-0 hidden group-hover/info:block w-72 p-3 bg-gray-900 text-white text-[11px] leading-snug rounded-lg shadow-xl z-[200] pointer-events-none whitespace-normal normal-case font-sans border border-gray-700">
      ...
      Сигналът се изчислява автоматично спрямо отклонението на <strong>Текущата цена</strong> от <strong>Справедливата</strong>:<br/><br/>
      <span className="text-yellow-300 font-mono text-[10px] block mb-2">IF(Отклонение &gt; {signalThreshold}% → Buy<br/>IF(Отклонение &lt; -{signalThreshold}% → Sell<br/>иначе → Hold)</span>
      <span className="text-emerald-400 font-bold">КУПУВАЙ</span> = Отклонение над +{signalThreshold}% (подценена!)<br/>
      <span className="text-amber-400 font-bold">ИЗЧАКАЙ</span> = Отклонение между двата прага<br/>
      <span className="text-rose-400 font-bold">ПРОДАВАЙ</span> = Отклонение под -{signalThreshold}% (надценена!)<br/><br/>
      Сигналът се изчислява <strong>автоматично</strong> — ръчна промяна не се запазва.<br/><br/>
