...
export default function BentoCharts({ stocks, activeFilter, onSetActiveFilter }: Props) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 mb-5">
      ...
      <div className="bg-[#1c1c1e] rounded-none border border-white/10 p-4 lg:col-span-2">
                ...
                <Tooltip
                  ...
                  content={({ active, payload }) => {
                    if (active && payload && payload.length) {
                      ...
                      return (
                        <div className="bg-[#1c1c1e] border border-white/10 rounded-none px-3 py-2 text-xs font-mono text-white">
                          <p className="font-extrabold mb-1 tracking-wider">{data.ticker}</p>
                          <p className="text-[#30d158]">Справедлива: ${data.fairPrice}</p>
                          <p className="text-gray-400">Текуща цена: ${data.currentPrice}</p>
      ...
      {/* 2. Signals Pie Chart */}
      <div className="bg-[#1c1c1e] rounded-none border border-white/10 p-4 flex flex-col justify-between lg:col-span-1">
        <div>
          <span className="text-[10px] text-white/60 font-serif italic uppercase tracking-wider block">
          ...
          <button
            ...
            className={`rounded-none py-1.5 px-0.5 border text-center transition-all active:scale-95 cursor-pointer flex flex-col items-center justify-center ${
              activeFilter.type === 'signal' && activeFilter.value === 'buy'
                ...
                : 'bg-[#1c1c1e] border-white/10/30 hover:bg-emerald-50/40 hover:border-emerald-650'
            }`}
            title="Филтрирай компании по Сигнал КУПУВАЙ"
          >
            <div className="text-[8px] text-gray-400 uppercase font-bold tracking-tight">КУПУВАЙ</div>
            <div className="text-xs font-extrabold text-[#30d158]">{signalBuyCount}</div>
          </button>
          <button
            ...
            className={`rounded-none py-1.5 px-0.5 border text-center transition-all active:scale-95 cursor-pointer flex flex-col items-center justify-center ${
              activeFilter.type === 'signal' && activeFilter.value === 'sell'
                ...
                : 'bg-[#1c1c1e] border-white/10/30 hover:bg-red-50/40 hover:border-red-650'
            }`}
            title="Филтрирай компании по Сигнал ПРОДАВАЙ"
          >
            <div className="text-[8px] text-gray-400 uppercase font-bold tracking-tight">ПРОДАВАЙ</div>
            <div className="text-xs font-extrabold text-[#ff3b30]">{signalSellCount}</div>
          </button>
          <button
            ...
            className={`rounded-none py-1.5 px-0.5 border text-center transition-all active:scale-95 cursor-pointer flex flex-col items-center justify-center ${
              activeFilter.type === 'signal' && activeFilter.value === 'hold'
                ...
                : 'bg-[#1c1c1e] border-white/10/30 hover:bg-amber-50/40 hover:border-amber-650'
            }`}
            title="Филтрирай компании по Сигнал ИЗЧАКАЙ"
          >
            <div className="text-[8px] text-gray-400 uppercase font-bold tracking-tight">ИЗЧАКАЙ</div>
      ...
      <div className="bg-[#1c1c1e] rounded-none border border-white/10 p-4 flex flex-col justify-between lg:col-span-1">
        ...
        <div className="grid grid-cols-3 gap-1 mt-1 font-mono">
          ...
          >
            <div className="text-[8px] text-gray-400 uppercase font-bold tracking-tight">BUY</div>
            <div className="text-xs font-extrabold text-[#30d158]">{bsBuyCount}</div>
          </button>
          <button
          ...
          >
            <div className="text-[8px] text-gray-400 uppercase font-bold tracking-tight">SELL</div>
            <div className="text-xs font-extrabold text-[#ff3b30]">{bsSellCount}</div>
          </button>
          <button
            onClick={() => handleFilterToggle('buySell', 'ДРУГИ')}
            className={`rounded-none py-1.5 px-0.5 border text-center transition-all active:scale-95 cursor-pointer flex flex-col items-center justify-center ${
              activeFilter.type === 'buySell' && activeFilter.value === 'ДРУГИ'
                ? 'bg-[#2c2c2e] border-gray-600 ring-2 ring-gray-600'
                : 'bg-[#1c1c1e] border-white/10/30 hover:bg-gray-50/40 hover:border-gray-600'
            }`}
            title="Филтрирай компании по ДРУГИ"
          >
            <div className="text-[8px] text-gray-400 uppercase font-bold tracking-tight">ДРУГИ</div>
            <div className="text-xs font-extrabold text-gray-300">{bsOthersCount}</div>
          </button>
        </div>
      </div>
