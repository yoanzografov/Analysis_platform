...
export default function CsvUploader({ onDataLoaded }: Props) {
  ...
  return (
    <div className="w-full bg-[#1c1c1e] border border-white/10 p-4 rounded-none shadow-xs space-y-4">
      ...
      {showHelp && (
        <div className="bg-[#D9D8D5] rounded-none p-3 text-xs text-white leading-relaxed space-y-2 border border-white/10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-[10px] font-mono text-white/80">
            <div className="space-y-1 bg-[#1c1c1e]/40 p-2 border border-white/10/15">
              <span className="font-black text-[#30d158] block uppercase">Метод A: Чрез Линк (Препоръчително)</span>
        ...
        <form onSubmit={handleUrlSync} className="md:col-span-5 flex flex-col justify-between border border-white/10/40 p-3 bg-stone-50">
          <div>
            <span className="text-[9px] uppercase font-bold text-gray-400 tracking-wider flex items-center gap-1">
              <Link2 className="w-3 h-3 text-[#30d158]" />
              Метод А: Директен ЛИНК
            </span>
            <p className="text-[10px] text-gray-300 leading-tight mt-1 font-serif">
            ...
            <input 
              ...
              className="w-full mt-2 p-1.5 text-[10px] font-mono border border-white/10 rounded-none focus:outline-none focus:bg-[#1c1c1e] bg-[#1c1c1e]/80"
            />
          </div>
        ...
        <div
          ...
          className={`md:col-span-7 py-4 px-3 rounded-none border border-dashed cursor-pointer transition-all duration-150 flex flex-col items-center justify-center text-center ${
            dragActive
              ...
              : 'border-white/10/40 bg-black/20 hover:bg-black/45 hover:border-white/10'
          }`}
        >
          <input
            ref={fileInputRef}
          ...
          {status === 'success' ? (
            <div className="flex flex-col items-center">
              <div className="p-1.5 rounded-none bg-emerald-700/10 text-[#30d158] mb-1.5 border border-emerald-700/40">
                <Check className="w-4 h-4" />
              </div>
              <p className="text-[11px] font-bold text-[#30d158] font-mono">Синхронизирането завърши успешно!</p>
              <p className="text-[9px] text-white/60 mt-0.5 font-mono italic">{fileName}</p>
            </div>
          ) : status === 'error' ? (
            <div className="flex flex-col items-center p-1">
              <div className="p-1 rounded-none bg-[#ff3b30]/10 text-[#ff3b30] mb-1 border border-red-700/40">
                <AlertCircle className="w-4 h-4" />
              </div>
              <p className="text-[10px] font-bold text-[#ff3b30] font-mono text-center max-w-xs">{errorMessage}</p>
              <p className="text-[8.5px] text-white/80 mt-1 hover:underline">Кликнете за да качите CSV в друг формат</p>
            </div>
          ) : (
            <div className="flex flex-col items-center">
              <div className="p-1.5 rounded-none bg-[#1c1c1e] border border-white/10/30 text-white mb-1.5 hover:bg-[#D9D8D5] transition-colors">
