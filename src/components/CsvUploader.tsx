...
export default function CsvUploader({ onDataLoaded }: Props) {
  ...
  return (
    <div className="w-full bg-white border border-[#141414] p-4 rounded-none shadow-xs space-y-4">
      {/* Header and Help Toggle */}
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <FileSpreadsheet className="w-4 h-4 text-emerald-700 shrink-0" />
          <h3 className="text-xs uppercase font-extrabold text-[#141414] tracking-wide font-mono">
            Синхронизиране с Google Sheets & CSV данни
          </h3>
        </div>
        <button
          onClick={() => setShowHelp(!showHelp)}
          className="text-[10px] text-[#141414]/70 hover:text-black flex items-center gap-1 font-mono transition-colors border border-[#141414]/30 px-2 py-0.5 bg-[#D9D8D5]/40 cursor-pointer"
        >
      ...
      {showHelp && (
        <div className="bg-[#D9D8D5] rounded-none p-3 text-xs text-[#141414] leading-relaxed space-y-2 border border-[#141414]">
          <p className="font-bold font-serif italic text-[11px]">Два начина за синхронизиране на вашите данни от Google Sheets:</p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-[10px] font-mono text-[#141414]/80">
            <div className="space-y-1 bg-white/40 p-2 border border-[#141414]/15">
              <span className="font-black text-emerald-800 block uppercase">Метод A: Чрез Линк (Препоръчително)</span>
          ...
          <p className="text-[9px] text-amber-900 font-mono font-bold mt-1">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-3.5">
        ...
        <form onSubmit={handleUrlSync} className="md:col-span-5 flex flex-col justify-between border border-[#141414]/40 p-3 bg-stone-50">
          <div>
            <span className="text-[9px] uppercase font-bold text-gray-500 tracking-wider flex items-center gap-1">
              <Link2 className="w-3 h-3 text-emerald-800" />
              Метод А: Директен ЛИНК
            </span>
            <p className="text-[10px] text-gray-700 leading-tight mt-1 font-serif">
            ...
            <input 
              ...
              className="w-full mt-2 p-1.5 text-[10px] font-mono border border-[#141414] rounded-none focus:outline-none focus:bg-white bg-white/80"
            />
          </div>
        ...
        <div
          ...
          className={`md:col-span-7 py-4 px-3 rounded-none border border-dashed cursor-pointer transition-all duration-150 flex flex-col items-center justify-center text-center ${
            dragActive
              ...
              : 'border-[#141414]/40 bg-[#E4E3E0]/20 hover:bg-[#E4E3E0]/45 hover:border-[#141414]'
          }`}
        >
          <input
            ref={fileInputRef}
          ...
          {status === 'success' ? (
            <div className="flex flex-col items-center">
              <div className="p-1.5 rounded-none bg-emerald-700/10 text-emerald-800 mb-1.5 border border-emerald-700/40">
                <Check className="w-4 h-4" />
              </div>
              <p className="text-[11px] font-bold text-emerald-800 font-mono">Синхронизирането завърши успешно!</p>
              <p className="text-[9px] text-[#141414]/60 mt-0.5 font-mono italic">{fileName}</p>
            </div>
          ) : status === 'error' ? (
            <div className="flex flex-col items-center p-1">
              <div className="p-1 rounded-none bg-red-100 text-red-800 mb-1 border border-red-700/40">
                <AlertCircle className="w-4 h-4" />
              </div>
              <p className="text-[10px] font-bold text-red-800 font-mono text-center max-w-xs">{errorMessage}</p>
              <p className="text-[8.5px] text-[#141414]/80 mt-1 hover:underline">Кликнете за да качите CSV в друг формат</p>
            </div>
          ) : (
            <div className="flex flex-col items-center">
              <div className="p-1.5 rounded-none bg-white border border-[#141414]/30 text-[#141414] mb-1.5 hover:bg-[#D9D8D5] transition-colors">
                <Upload className="w-3.5 h-3.5 text-[#141414]" />
              </div>
              <p className="text-[10px] text-[#141414] font-bold">
                Метод Б: Плъзнете своя <span className="underline font-mono">.csv</span> файл тук
              </p>
              <p className="text-[8.5px] text-[#141414]/55 font-mono mt-0.5 uppercase">
