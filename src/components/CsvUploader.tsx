...
export default function CsvUploader({ onDataLoaded }: Props) {
 ...
 <h3 className="text-xs uppercase font-extrabold text-ink tracking-wide font-sans tabular-nums">
 Синхронизиране с Google Sheets & CSV данни
 </h3>
 </div>
 <button
 onClick={() => setShowHelp(!showHelp)}
 className="text-xs text-ink/70 hover:text-ink flex items-center gap-1 font-sans tabular-nums transition-colors border border-border/30 px-2 py-0.5 bg-[#D9D8D5]/40 cursor-pointer rounded-2xl"
 >
 ...
 <p className="font-bold font-serif italic text-xs">Два начина за синхронизиране на вашите данни от Google Sheets:</p>
 <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs font-sans tabular-nums text-ink/80">
 ...
 <p className="text-xs text-amber-400 font-sans tabular-nums font-bold mt-1">
 * Платформата поддържа оригиналната структура с показатели като "Fair Price" и "Current Price" и автоматично преизчислява формулите.
 ...
 <div>
 <span className="text-xs uppercase font-bold text-ink-faint tracking-wider flex items-center gap-1">
 ...
 className="w-full mt-2 p-1.5 text-xs font-sans tabular-nums border border-border focus:outline-none focus:bg-card rounded-2xl bg-card"
 />
 </div>
 ...
 className={`w-full mt-3.5 py-1.5 px-3 uppercase tracking-wider text-xs font-sans tabular-nums font-bold border border-black flex items-center justify-center gap-2 rounded-2xl transition-all ${
 !sheetUrl.trim() 
 ...
 <p className="text-xs font-bold text-[#10b981] font-sans tabular-nums">Синхронизирането завърши успешно!</p>
 <p className="text-xs text-ink/60 mt-0.5 font-sans tabular-nums italic">{fileName}</p>
 </div>
 ) : status === 'error' ? (
 ...
 <p className="text-xs font-bold text-[#f43f5e] font-sans tabular-nums text-center max-w-xs">{errorMessage}</p>
 ...
 Метод Б: Плъзнете своя <span className="underline font-sans tabular-nums">.csv</span> файл тук
 </p>
 <p className="text-[8.5px] text-ink/55 font-sans tabular-nums mt-0.5 uppercase">
